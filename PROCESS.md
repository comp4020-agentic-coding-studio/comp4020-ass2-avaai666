...
Rule 7 bans "delve", "tapestry", and other words; `spec/voice.test.ts`
checks the built HTML for them. One entry, "in today's", could never fire:
the renderer turns a straight apostrophe into a curly one, and the list
used a straight one — green, and proved nothing. The obvious fix was a
curly-quote copy of the phrase in the list.
...
match let "dive" inside "divided" or "division" fail ordinary sentences.
The obvious move was another
...
Reading `dist/` found the opposite: only bare `.md`/`.mdx` pages with no
`layout:` frontmatter reach it, not routes calling `ContentLayout` directly
— and that claim had cost three turns on a wrong spacing fix. The
obvious move was to fix my import and move on.
...
The hero alt text drifted from the picture it described, twice, fixed by
hand each time
...
The third time it was the card's description, still naming artwork two
versions back, while every check stayed green: axe checks that alt text
exists, not that it's true. The obvious move was to rewrite the sentence
again.
...
it quietly rendered twice at every width, because a rule meant to hide one
variant lost the cascade to a higher-specificity selector. All four only
ask whether a mark appears in the HTML; two copies satisfy that as well as
one. The obvious move was to delete one of the two drawings.
